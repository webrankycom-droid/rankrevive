...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    // Verify ownership and get page data
    ...
    // Update page with optimized content and score
    await query(
      `UPDATE pages SET
         optimized_content = $1,
         content_score = $2,
    ...
    // Increment pages used this month
    await query(
...
router.put(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const result = await wordpressService.publishOptimizationToWordPress(
      page.site_id,
      optimization.page_id,
    ...
    if (!result.success) {
      res.status(500).json({ error: result.error || 'Failed to publish to WordPress' });
      return;
    }

    await query(
      'UPDATE optimizations SET status = $1, published_at = NOW(), wp_post_id = $2 WHERE id = $3',
      [status === 'publish' ? 'published' : 'draft_wp', result.wpPostId, optimizationId]
    );

    if (status === 'publish') {
      await query('UPDATE pages SET status = $1 WHERE id = $2', ['published', optimization.page_id]);
    }
...
router.get('/history/list', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const countResult = await queryOne<{ count: string }>(
    'SELECT COUNT(*) as count FROM optimizations WHERE user_id = $1',
    [req.user!.userId]
  );
